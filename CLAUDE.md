...
- 已知变量：`VITE_TIANDITU_KEY`（天地图浏览器端 Key，申请地址 <<https://console.tianditu.gov.cn/api/key）、`VITE_CESIUM_ION_ACCESS_TOKEN`（Cesium> ion 访问令牌，用于加载 Cesium World Terrain 地形，申请地址 <https://ion.cesium.com/tokens>）、`VITE_MAP_ENGINE`（仅供 UI 标签，模式由 Vite mode 决定）；后端脚本专用：`RUSTFS_ACCESS_KEY` / `RUSTFS_SECRET_KEY`（`scripts/upload_terrain.py` 读取的 RustFS/S3 凭据，不带 `VITE_` 前缀，由 Python 直接通过 `os.getenv` 读取）。
